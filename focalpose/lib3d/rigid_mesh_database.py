...
from focalpose.utils.tensor_collection import TensorCollection


class MeshDataBase:
    def __init__(self, obj_list):
        self.infos = {obj['label']: obj for obj in obj_list}
        self.meshes = {l: scene_as_mesh(trimesh.load(Path(obj['urdf_path']).with_suffix('.obj'))) for l, obj in self.infos.items()}

    @staticmethod
    def from_urdf_ds(urdf_ds):
        obj_list = [urdf_ds.index.iloc[n] for n in range(len(urdf_ds))]
    ...
    def batched(self, aabb=False, resample_n_points=None, n_sym=64):
        ...
        for label, mesh in self.meshes.items():
            ...
            symmetries_n = make_bop_symmetries(dict_symmetries, n_symmetries_continuous=n_sym, scale=1.0)
