...
def add_noise_f(K, f_std_frac=0.15):
    K_out = K.clone()
    device = K_out.device
    bsz = K.shape[0]
    focal_noise = torch.tensor(np.concatenate(np.random.normal(loc=0, scale=f_std_frac*K[:, 0, 0].cpu().numpy(), size=bsz)[:, None])).float().to(device)
    K_out[:, 0, 0] += focal_noise
    K_out[:, 1, 1] += focal_noise
    return K_out
