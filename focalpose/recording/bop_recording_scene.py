...
from focalpose.config import ASSET_DIR

from focalpose.datasets.datasets_cfg import make_urdf_dataset, make_texture_dataset

from focalpose.simulator import BaseScene, Body, Camera
from focalpose.simulator import BodyCache, TextureCache, apply_random_textures
...
class BopRecordingScene(BaseScene):
    ...
    def sample_camera(self):
        assert self.focal_interval.shape == (2, )
        K = np.zeros((3, 3), dtype=np.float)
        f = self.np_random.uniform(*self.focal_interval)
